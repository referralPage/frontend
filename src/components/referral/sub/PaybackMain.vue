<template>
  <div class="payback_main_wrap pr">
    ...
    <ul class="exchange_area flex_col_c">
      <li class="flex_row_c_c" v-for="exchange in exchangeList" :key="exchange.name" @click="enrollCheck(exchange)">
        <img class="logo_s" :src="exchange.logo" :alt="`${exchange.name} logo`"/>
        <img class="logo_l" :src="exchange.text" :alt="`${exchange.name} logo`"/>
    </li>
    </ul>
  </div>
</template>
...
const emit = defineEmits(["receiveExchange"]);
const enrollCheck = (exchange) => {
    // console.log(exchange);

    //등록됐는지 체크하고, 등록됐을 시 '이미 등록되었습니다.' 팝업 띄우기
    // 안됐을 시 페이백 신청 페이지로 변경
    //emit으로 부모요소에 상태값하나 보내주기.
    emit("receiveExchange",exchange,true);
}
</script>
